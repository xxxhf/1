    ...
    if (strcmp(argv[0], "cd") == 0) {
        ...
        if(argv >= 4){
            ...
            return -1;
                ...
                flag2 = kill(pid,SIGTERM);
                if(flag2 == 0) return 0;
                else return -1;
        }
        else{
            printf("请为kill配置足够的参数\n");
            return -1;
        }
        return -1;
    }
...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc){
        ...
        if(strcmp(argv[i], ">") == 0) {
            //TODO: 打开输出文件从头写入
            tfd = open(argv[i + 1] ,O_CREAT | O_WRONLY, 0664);
            if(tfd < 0) {
            ...
            //TODO: 打开输出文件追加写入
            tfd = open(argv[i+1], O_CREAT | O_WRONLY | O_APPEND, 0664);
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } 
            else {
...
int main() {
    /* 输入的命令行 */
    char cmdline[MAX_CMDLINE_LENGTH];//1024

    char *commands[128];//多个子命令
    int cmd_count;

    char *argv[MAX_CMD_ARG_NUM];//参数表列
    int argc;//参数个数
    int fd[2];
    int pid;
    ...
    while (1) {
        ...
        printf("shell@xhf:%s-> ",path);
        fflush(stdout);

        fgets(cmdline, 256, stdin);//标准输入
        strtok(cmdline, "\n");//按照换行符对指令进行分割，获取第一条
        ...
        if(cmd_count == 0) {
            ...
            if(pid == 0) {  
                ...
                dup2(pipefd[WRITE_END], STDOUT_FILENO);//默认输出
                ...
                int argc = split_string(commands[0], " ", argv);//提取第一条命令
                execute(argc, argv);
                exit(255);
